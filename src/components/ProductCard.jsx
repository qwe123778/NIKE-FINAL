...
import { Heart, ArrowUpRight, Store } from "lucide-react";
    ...
    >
      <Link to={`/product/${product.id}`} className="flex flex-col h-full">
        <div className="relative aspect-[4/5] overflow-hidden">
          ...
          {product.isNew && (
            <span className="absolute top-4 left-4 font-mono-tech bg-primary text-primary-foreground px-2.5 py-1 text-[10px] z-10">
              New
            </span>
          )}
        ...
        <div className="flex flex-col flex-1 px-5 pt-5 pb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            {/* Seller name — uses button + navigate to avoid nested <a> */}
            {product.seller_id && (
              <button
                onClick={goToSellerProfile}
                className="flex items-center gap-1 font-mono-tech text-[10px] text-muted-foreground hover:text-foreground transition-colors"
              >
          ...
          <h3 className="font-display text-xl not-italic leading-tight mb-2 line-clamp-2">
            {product.name}
          </h3>
